...
- `resyncSheetInPlace` corrects the **sheet's own** `counterAxisSizingMode` back to `FIXED` if it's drifted to `AUTO`/HUG, preserving its current width exactly (only the sizing *mode* is fixed, not the pixel value) — a FILL child silently fails to apply (caught, swallowed) if its parent isn't actually in FIXED mode, so a sheet stuck on HUG would make every section inside it stay stuck too, no matter how correct the per-section FILL assignment is.
- `resyncSectionInPlace` (used for a section resynced on its own, without its sheet in scope) only preserves a section's existing sizing/width when it's inside a frame the *user* built — a section still living inside one of our own sheets always gets `FILL`, matching `resyncSheetInPlace`. Without this distinction, a section that was HUG due to a bug fixed since would stay HUG forever: each resync would faithfully copy forward whatever the previous (broken) state was.
